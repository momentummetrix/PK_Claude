#!/usr/bin/env python3
"""
Quality Scoring System for Course Materials and NONMEM Control Streams
...
Usage:
    ...
    python scripts/quality_score.py models/run101/run101.mod
"""
...
NONMEM_RUBRIC = {
    'critical': {
        'theta_zero': {'points': 100, 'auto_fail': True},
        'missing_estimation': {'points': 100, 'auto_fail': True},
        'missing_data': {'points': 100, 'auto_fail': True},
        'missing_problem': {'points': 50},
        'missing_table': {'points': 20},
    },
    'major': {
        'missing_covariance': {'points': 15},
        'unbounded_theta': {'points': 10},
        'missing_omega_block': {'points': 5},
        'hardcoded_path': {'points': 10},
    },
    'minor': {
        'missing_comment': {'points': 2},
        'non_standard_naming': {'points': 1},
    }
}
...
class IssueDetector:
    ...
    @staticmethod
    def check_nonmem_control_stream(content: str) -> Dict[str, List]:
        """Check NONMEM control stream for common issues.

        Returns dict with 'critical', 'major', 'minor' issue lists.
        """
        ...
        # Check for $PROBLEM
        if not re.search(r'^\s*\$PROB', content, re.MULTILINE):
            issues['critical'].append({
                'type': 'missing_problem',
                'description': '$PROBLEM statement missing',
                'details': 'Every control stream must start with $PROBLEM',
                'points': 50
            })

        # Check for $DATA
        if not re.search(r'^\s*\$DATA', content, re.MULTILINE):
            issues['critical'].append({
                'type': 'missing_data',
                'description': '$DATA statement missing',
                'details': 'Control stream must specify input dataset',
                'points': 100
            })

        # Check for $ESTIMATION
        if not re.search(r'^\s*\$EST', content, re.MULTILINE):
            issues['critical'].append({
                'type': 'missing_estimation',
                'description': '$ESTIMATION statement missing',
                'details': 'Control stream must include estimation method',
                'points': 100
            })

        # Check for THETA initial estimate = 0 (auto-fail)
        theta_section = False
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
            if re.match(r'\$THETA', stripped, re.IGNORECASE):
                theta_section = True
                continue
            if theta_section and stripped.startswith('$'):
                theta_section = False
            if theta_section and stripped and not stripped.startswith(';'):
                # Match patterns like: 0 ; comment, (lower, 0, upper), 0 FIXED
                # Extract numeric values from THETA lines
                clean = stripped.split(';')[0].strip()
                if not clean:
                    continue
                # Check for bare 0 or (low, 0, high) patterns
                nums = re.findall(r'[-+]?\d*\.?\d+(?:[eE][-+]?\d+)?', clean)
                # For (low, init, high) format, check the middle value
                paren_match = re.match(r'\(\s*[-+]?\d*\.?\d*\s*,\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s*(?:,\s*[-+]?\d*\.?\d*)?\s*\)', clean)
                if paren_match:
                    init_val = float(paren_match.group(1))
                    if init_val == 0.0:
                        issues['critical'].append({
                            'type': 'theta_zero',
                            'description': f'THETA initial estimate = 0 at line {i}',
                            'details': 'THETA initial estimates can NEVER be zero. Use 0.00001 instead.',
                            'points': 100
                        })
                elif nums and not re.match(r'\(', clean):
                    # Bare value (not in parentheses)
                    first_val = float(nums[0])
                    if first_val == 0.0 and 'FIX' not in clean.upper():
                        issues['critical'].append({
                            'type': 'theta_zero',
                            'description': f'THETA initial estimate = 0 at line {i}',
                            'details': 'THETA initial estimates can NEVER be zero. Use 0.00001 instead.',
        ...
        # Check for $TABLE
        if not re.search(r'^\s*\$TABLE', content, re.MULTILINE):
            issues['critical'].append({
                'type': 'missing_table',
                'description': '$TABLE statement missing',
                'details': 'Output tables needed for diagnostics (IPRED, CWRES, etc.)',
                'points': 20
            })
        ...
        if not re.search(r'^\s*\$COV', content, re.MULTILINE):
            issues['major'].append({
                'type': 'missing_covariance',
                'description': '$COVARIANCE step not requested',
                'details': 'Both convergence AND covariance must succeed for model acceptance',
                'points': 15
            })

        # Check for unbounded THETAs (no parentheses = no bounds)
        theta_section = False
        for i, line in enumerate(lines, 1):
            stripped = line.strip()
            if re.match(r'\$THETA', stripped, re.IGNORECASE):
            ...
            if theta_section and stripped and not stripped.startswith(';'):
                clean = stripped.split(';')[0].strip()
                if clean and not re.match(r'\(', clean) and 'FIX' not in clean.upper():
                    issues['major'].append({
                        'type': 'unbounded_theta',
                        'description': f'Unbounded THETA at line {i}',
                        'details': 'Use (lower, init, upper) format for parameter bounds',
                        'points': 10
                    })

        # Check for hardcoded absolute paths in $DATA
        data_match = re.search(r'\$DATA\s+(\S+)', content)
        if data_match:
            data_path = data_match.group(1)
            if data_path.startswith('/') or re.match(r'[A-Za-z]:', data_path):
                issues['major'].append({
                    'type': 'hardcoded_path',
                    'description': f'Hardcoded absolute path in $DATA: {data_path}',
                    'details': 'Use relative paths for portability',
                    'points': 10
                })
        ...
        has_comment = any(line.strip().startswith(';') for line in lines[:10])
        if not has_comment:
            issues['minor'].append({
                'type': 'missing_comment',
                'description': 'No documentation comments in first 10 lines',
                'details': 'Add ; comments describing model purpose and structure',
                'points': 2
            })
...
class QualityScorer:
    ...
    def score_nonmem(self) -> Dict:
        """Score NONMEM control stream quality."""
        content = self.filepath.read_text(encoding='utf-8')

        nm_issues = IssueDetector.check_nonmem_control_stream(content)

        for issue in nm_issues['critical']:
            self.issues['critical'].append(issue)
            if issue['type'] in ('theta_zero', 'missing_estimation', 'missing_data'):
                self.auto_fail = True
            self.score -= issue['points']

        for issue in nm_issues['major']:
            self.issues['major'].append(issue)
            self.score -= issue['points']

        for issue in nm_issues['minor']:
  ...
  2 = Auto-fail (compilation/syntax error)
    ...
    for filepath in args.filepaths:
        ...
        try:
            ...
            elif filepath.suffix in ('.mod', '.ctl'):
                report = scorer.score_nonmem()
            else:
