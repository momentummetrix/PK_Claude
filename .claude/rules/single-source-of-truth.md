...
```
NONMEM .lst (SOURCE OF TRUTH for model results)
  ├── xpose_data() → xpdb object (derived, in R)
  ├── Parameter tables → reports (derived)
  ├── GOF plots → reports/figures/ (derived)
  ├── model_development_log.yaml (metadata, manually curated)
  └── control stream .mod (SOURCE OF TRUTH for model specification)

NEVER manually type parameter values into reports.
ALWAYS extract from .lst via xpose or aipharma.
```
